Shader "VolumeRendering/SphereMarchingVolumeShader"
{
	Properties
	{
		...
		_StepJump("StepJump", Range(1.0, 100.0)) = 5
	}
		CGINCLUDE
			...
			SubShader{
				...
				Pass
				{
					...
					#define AmountOfSegments 4
					#endif

					half4 _Color;
					half4 _Colors[AmountOfSegments];
					float _max[AmountOfSegments];
					float _min[AmountOfSegments];
					sampler3D _Volume;
					...
					float sample_volume(float tex, float3 p)
					{
					  ...
					  float step_size = 1 / float(ITERATIONS);
					  ...
					  for (int iter = 0; iter < 8; iter++)
					  {
						...
						float nextStep = max(tex.r, 1);

						// get the darkness of the value
						float v = sample_volume(1-tex.a, p);
						
						float c = max(dst.a, v);
						dst = float4(c, c, c, c);

						p += ds * max((nextStep * 100), 1);
